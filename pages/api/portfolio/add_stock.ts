...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const stockInfo = await fetch(
      `https://query1.finance.yahoo.com/v8/finance/chart/${String(ticker)}`,
    );
    const stockInfoJson: any = await stockInfo.json();
    if (
      stockInfoJson.chart.result === undefined ||
      stockInfoJson.chart.result === null
    ) {
      res.status(403);
      res.json({
    ...
    const conversionRate = await getConversionRate(
      stockInfoJson.chart.result[0].meta.currency,
      settingsCurrency,
    );
    ...
    const prevClose = parseFloat(
      (
        stockInfoJson.chart.result[0].meta.regularMarketPrice * conversionRate
      ).toFixed(2),
    );
    ...
    const { newStocks, newNetWorth, newTotalInvested } = await addStock(
      newStock,
      email,
    );

    res.json({
      stocks: newStocks.stocks,
