...
export const sortStocks = (orderBy: string, stocksInput: StockInterface[]) => {
  if (orderBy === 'A-Z') {
    ...
    stocksInput.sort(function (a, b) {
      return b.prevClose * b.amount - a.prevClose * a.amount;
    });
  } else if (orderBy === 'VALUE LOW') {
    stocksInput.sort(function (a, b) {
      return a.prevClose * a.amount - b.prevClose * b.amount;
    });
  } else if (orderBy === 'CHANGE HIGH') {
    stocksInput.sort(function (a, b) {
      return b.avgPercentageChange - a.avgPercentageChange;
    });
  } else if (orderBy === 'CHANGE LOW') {
    stocksInput.sort(function (a, b) {
      return a.avgPercentageChange - b.avgPercentageChange;
    });
  }
};
