...
function transformStocks(stocks: StockInterface[]): StockInterface[] {
  if (stocks.length === 0) return [];

  const etfs = stocks.filter((s) => s.quoteType === 'ETF');
  const others = stocks.filter((s) => s.quoteType !== 'ETF');

  return [aggregateStocks(etfs, 'ETF'), aggregateStocks(others, 'INDIVIDUAL')];
}
...
const Portfolio = () => {
  ...
  useEffect(() => {
    fetch('/api/portfolio/stocks', {
      ...
      .then((returnedStocks) => {
        ...
        const portfolioStocks: PortfolioStockInterface[] = returnedStocks.map(
  ...
  const sortedStocks = getFilteredStocks()
    .slice()
    .sort((a, b) => {
      if (orderBy) {
  ...
  return (
    <Grid>
            ...
            <TableRow>
              ...
              <TableCell align="center">
                <TableSortLabel
                  ...
                  onClick={() => {
                    handleSort('avgPercentageChange');
                  }}
                >
                  Avg Trade Change
                </TableSortLabel>
              ...
              <TableCell align="center">
                <TableSortLabel
                  ...
                  direction={
                    orderBy === 'regularMarketChangePercent' ? order : 'asc'
                  }
                  onClick={() => {
                    handleSort('regularMarketChangePercent');
                  }}
                >
              ...
              <TableCell align="center">
                <TableSortLabel
                  ...
                  direction={
                    orderBy === 'fiftyTwoWeekChangePercent' ? order : 'asc'
                  }
                  onClick={() => {
                    handleSort('fiftyTwoWeekChangePercent');
                  }}
                >
                  1yr Change
                </TableSortLabel>
              ...
              <TableCell align="center">
                <TableSortLabel
                  ...
                  onClick={() => {
                    handleSort('dividendRatePercent');
                  }}
                >
                  Dividend
                </TableSortLabel>
            ...
            {sortedStocks.map((stock) => (
              ...
              >
                ...
                <TableCell
                  ...
                  sx={{
                    color:
                      stock.fiftyTwoWeekChangePercent >= 0 ? 'green' : 'red',
                  }}
                >
                  {Math.round(stock.fiftyTwoWeekChangePercent * 100) / 100}%
                </TableCell>
                <TableCell align="center">
                  {Math.round(stock.trailingPE)}
                </TableCell>
                <TableCell align="center">
                  {Math.round(stock.forwardPE)}
                </TableCell>
                <TableCell align="center">
                  {Math.round(stock.dividendRatePercent * 100) / 100}%
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Grid display="flex" justifyContent="center" alignItems="center" mx={50}>
