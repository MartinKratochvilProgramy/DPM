...
export const OrderDropDown: React.FC<Props> = ({
  ...
  const [dropdownState, setDropdownState] = useState(orderDropdownValue)
  ...
  return (
    <>
      <button
        ...
        className={`relative flex flex-row min-w-[100px] border border-solid border-blue-600 justify-center items-center py-1 px-3 font-medium text-[12px] xsm:text-xs leading-snug rounded-lg whitespace-nowrap shadow-sm hover:shadow-md transition duration-150 ease-in-out ${themeStyles}`}
        type="button">
        {dropdownState}
        <svg
