...
export const Stock: React.FC<Props> = ({ demo, stock, deletePurchase }) => {
  ...
  function openDeleteStockModal(
    ...
    purchaseIdToDelete: number | null,
  ) {
    e.stopPropagation();
      ...
      >
        <div className="flex flex-row w-full items-center justify-between mb-1">
          <div className="flex flex-row justify-start sm:justify-start w-full max-w-[400px] px-1 dark:text-gray-100">
            <div className=" w-[45px] sm:w-[46px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex justify-start font-bold">
              {stock.ticker}
            </div>
            <div className=" w-[45px] sm:w-[46px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex justify-center">
              {stock.amount}
            </div>
            <div className=" w-[45px] sm:w-[46px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex justify-center">
          ...
          <div
            onClick={(e) => {
              openDeleteStockModal(e, stock.amount, null);
            }}
            id={stock.ticker}
        ...
        {expanded && (
          <div className="flex flex-col items-start justify-start border-t-[1px] border-t-gray-300 dark:border-t-gray-500">
            <div className="flex flex-row justify-start sm:justify-start w-full max-w-[400px] mt-4 mb-1 px-1 dark:text-gray-100">
              <div className="w-[45px] sm:w-[48px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex justify-start">
                DATE
              </div>
              <div className="w-[45px] sm:w-[48px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex justify-center">
                AMOUNT
              </div>
              <div className="w-[45px] sm:w-[48px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex justify-center">
                PRICE
            ...
            {stock.purchases.map((purchase: PurchaseInterface, i) => {
              ...
              return (
                <div
                  key={purchase.id}
                  className="flex flex-row w-full justify-start sm:justify-between items-center rounded-2xl pr-1 py-1 hover:bg-gray-100 dark:hover:bg-opacity-5 mr-1"
                >
                  <div className="flex text-[10px] sm:text-[12px] flex-row w-full max-w-[400px] px-1 justify-start sm:justify-start rounded-xl h-full dark:text-gray-300">
                    <div className="w-[45px] sm:w-[48px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex items-center justify-start">
                      {day}-{month}-{year}
                    </div>
                    ...
                    <div className="w-[45px] sm:w-[48px] md:w-[60px] xl:w-[64px] 2xl:w-[74px] flex items-center justify-center">
                      {purchase.relativeChange >= 0 ? (
                        <div className="text-green-600">
                          {'+' + purchase.relativeChange.toFixed(1) + '%'}
                        </div>
                      ) : (
                  ...
                  <div
                    onClick={(e) => {
                      openDeleteStockModal(e, purchase.amount, purchase.id);
                    }}
                    id={stock.ticker}
            ...
            <div className="flex w-full justify-center items-center gap-2 mt-1">
              <OrderDropDown
                values={['6m', '1y', '2y', '5y']}
                orderDropdownValue={period}
                setOrderDropdownValue={setPeriod}
                handleClick={handleDropdownClick}
                theme={'light'}
              />
              <button
                onClick={(e) => {
                  handleChartDisplay(e);
                }}
